...
type SubscriptionConfig[T any] struct {
	...
	// sent on the topic.
	//
	// To reference a method on an [KhulnaSoft service struct]
	...
	//
	// [KhulnaSoft service struct]: https://khulnasoft.com/docs/primitives/services-and-apis/service-structs
	...
	//
	// This setting also has no effect on KhulnaSoft Cloud environments.
	//
...
type RetryPolicy struct {
	...
	//   n > 0:  KhulnaSoft will forward a message to a dead letter queue after n retries
	//   n == pubsub.InfiniteRetries: Messages will not be forwarded to the dead letter queue by the KhulnaSoft framework
	MaxRetries int
}
