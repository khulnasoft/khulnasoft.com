...
//
// For more information about KhulnaSoft's cron job support, see https://khulnasoft.com/docs/develop/cron-jobs.
package cron

import "os"

// NewJob defines a new cron job. It is specially recognized by the KhulnaSoft Parser
// and results in the KhulnaSoft Platform provisioning the cron job on next deploy.
...
// refactor the code and move the cron job definition to another package, KhulnaSoft uses
...
// directly by the KhulnaSoft Platform and are not actually executed at runtime.
//
// To define a new cron job, call NewJob and assign it a package-level variable:
//
//	import "khulnasoft.com/cron"
//
...
func NewJob(id string, jobConfig JobConfig) (_ *Job) {
	...
	//    https://github.com/khulnasoft/khulnasoft/blob/v1.41.9/runtimes/go/cron/cron.go#L39-L47
...
type JobConfig struct {
	...
	// Endpoint is the KhulnaSoft API endpoint that should be called when the cron job executes.
	...
	//
	// The KhulnaSoft compiler will catch this and give you a helpful error at compile-time if you try to use an invalid interval.
	Every Duration
