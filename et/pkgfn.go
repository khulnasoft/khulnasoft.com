...
import (
	...
	"khulnasoft.com/beta/auth"
	"khulnasoft.com/storage/sqldb"
)
...
func OverrideAuthInfo(uid auth.UID, data any) {
	...
	//    https://github.com/khulnasoft/khulnasoft/blob/v1.41.9/runtimes/go/et/pkgfn.go#L29-L31
...
func EnableServiceInstanceIsolation() {
	...
	//
	// The current implementation of this function can be found here:
	//    https://github.com/khulnasoft/khulnasoft/blob/v1.41.9/runtimes/go/et/pkgfn.go#L42-L44
	doPanic("khulnasoft apps must be run using the khulnasoft command")
	return
}
...
// The database name must be a database known to KhulnaSoft (via `sqldb.NewDatabase`),
...
func NewTestDatabase(ctx context.Context, name stringLiteral) (_ *sqldb.Database, _ error) {
	...
	//
	// The current implementation of this function can be found here:
	//    https://github.com/khulnasoft/khulnasoft/blob/v1.41.9/runtimes/go/et/pkgfn.go#L61-L63
	doPanic("khulnasoft apps must be run using the khulnasoft command")
	return
}
